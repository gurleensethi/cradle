...
func Init() error {
	...
	instance.CradleConfig = cradleConfig
...
func parseCradleConfigFile(configFilePath string) (CradleConfig, error) {
	...
	slices.SortFunc(instance.CradleConfig.Projects, func(a, b types.CradleProject) int {
...
// UpdateConfigFile writes the current configuration to the config file.
func UpdateConfigFile() error {
	slices.SortFunc(instance.CradleConfig.Projects, func(a, b types.CradleProject) int {
		return strings.Compare(a.Path, b.Path)
	})

	fileBytes, err := toml.Marshal(instance.CradleConfig)
	if err != nil {
		return err
	}
